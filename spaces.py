...
def unload_module():
    ...
    print(f'Перемещение модуля в CPU: {type(module_in_gpu).__name__}')
...
def greedy_move_to_gpu(model, model_gpu_memory_when_using_cpu_swap):
    ...
    print(f"[Управление памятью] Загружено в CPU Swap: {memory_in_swap / (1024 * 1024):.2f} MB")
    print(f"[Управление памятью] Загружено в GPU: {mem_counter / (1024 * 1024):.2f} MB")
    return
...
def load_module(m):
    ...
    print(f"[Управление памятью] Свободно памяти GPU: {current_free_mem / (1024 * 1024):.2f} MB")
    print(f"[Управление памятью] Требуется памяти для модели: {model_memory / (1024 * 1024):.2f} MB")
    print(f"[Управление памятью] Требуется памяти для инференса: {inference_memory / (1024 * 1024):.2f} MB")
    print(f"[Управление памятью] Ожидаемый остаток памяти GPU: {estimated_remaining_memory / (1024 * 1024):.2f} MB")

    is_torch_jit = 'ScriptModule' in type(m).__name__

    if is_torch_jit:
        print(f'Обнаружен Torch JIT модуль: {type(m).__name__}')

    if (ALWAYS_SWAP or estimated_remaining_memory < 0) and not is_torch_jit:
        print(f'Перемещение модуля в SWAP: {type(m).__name__}')
        ...
        print(f'Перемещение модуля в GPU: {type(m).__name__}')
        m.to(gpu)
...
class GPUObject:
    ...
    def to(self, device):
        for module in self.module_list:
            module.to(device)
        print(f'Forge Space: Перемещено {len(self.module_list)} модулей на {device}')
        return self
...
def GPU(gpu_objects=None, manual_load=False, **kwargs):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            print("Вход в Forge Space GPU ...")
            memory_management.unload_all_models()
            if not manual_load:
                for o in gpu_objects:
                    o.gpu()
            result = func(*args, **kwargs)
            print("Очистка Forge Space GPU ...")
            unload_module()
            for o in gpu_objects:
...
def download_single_file(
    ...
    *,
    ...
    if not os.path.exists(cached_file):
        ...
        print(f'Загрузка: "{url}" в {cached_file} с использованием временного файла {tmp_filename}\n')
...
def automatically_move_to_gpu_when_forward(m: torch.nn.Module, target_model: torch.nn.Module = None):
    ...
    def patch_method(method_name):
        ...
        if method_name in m.forge_space_hooked_names:
            print(f'Уже подключен хук {type(m).__name__}.{method_name}')
            return

        print(f'Автоматический хук: {type(m).__name__}.{method_name}')
